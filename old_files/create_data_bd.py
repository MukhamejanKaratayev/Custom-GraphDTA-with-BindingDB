...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        ...
        df = pd.read_csv('data/bindingdb/bindingDB_' + dataset + '_test.csv')
        test_drugs, test_prots,  test_Y = list(df['Drug']),list(df['Target']),list(df['Y'])
        XT = [seq_cat(t) for t in test_prots]
        test_drugs, test_prots,  test_Y = np.asarray(test_drugs), np.asarray(XT), np.asarray(test_Y)

        # make data PyTorch Geometric ready
        print('preparing bindingdb_', dataset + '_train.pt in pytorch format!')
        train_data = TestbedDataset(root='data/bindingdb', dataset='bindingDB_'+dataset+'_train', xd=train_drugs, xt=train_prots, y=train_Y,smile_graph=smile_graph)
        print('preparing bindingdb_', dataset + '_test.pt in pytorch format!')
        test_data = TestbedDataset(root='data/bindingdb', dataset='bindingDB_'+dataset+'_test', xd=test_drugs, xt=test_prots, y=test_Y,smile_graph=smile_graph)
